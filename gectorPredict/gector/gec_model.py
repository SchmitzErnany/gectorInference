...
from gectorPredict.gector.bert_token_embedder import PretrainedBertEmbedder
from gectorPredict.gector.seq2labels_model import Seq2Labels
    ...
    def postprocess_batch(
        ...
        for tokens, probabilities, idxs, error_prob in zip(
            ...
            if max(idxs) == 0:
                ...
                logger_only_wrongs.info(
                    f"no errors identified in the sentence: {tokens}"
                )
                continue
            ...
            if max(error_prob) < lowest_min_value:
                ...
                all_transforms.append([""] * len(idxs))
                logger_all.info(
                    f"errors lie below the min_error_probability threshold in the sentence: {tokens}"
                )
                logger_only_wrongs.info(
                    f"errors lie below the min_error_probability threshold in the sentence: {tokens}"
                )
